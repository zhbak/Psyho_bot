    ...
    async def query_handler(call):
        ...
        if call.data == 'pushed_start_psychat_btn':
            ...
            if user_info.remaining_sessions_count > 0: # Если есть доступные сессии
                ...
                response = await psy_chat.psyho_chat(str(prompts.system_prompt), str(user_input), database.pool, str(chat_id), chat_history, config.chat) # Ответ psychat на первый user_input
